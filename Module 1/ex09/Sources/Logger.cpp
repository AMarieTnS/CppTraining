...
void Logger::logToConsole(std::string const &sLogMessageConsole) {
  std::cout << sLogMessageConsole << std::endl;
}
...
void Logger::logToFile(std::string const &sLogMessageFile) {
  ...
  MyFile << sLogMessageFile;
  MyFile.seekg(0, std::ios::beg);

  std::string sContent((std::istreambuf_iterator<char>(MyFile)),
                       std::istreambuf_iterator<char>());
  std::cout << "Content after writing logs in file: " << sContent << std::endl;
  std::cout << sLogMessageFile << std::endl;
}
...
void Logger::log(std::string const &dest, std::string const &message) {
  ...
  std::cerr << "Unknown action: " << message << std::endl;
}